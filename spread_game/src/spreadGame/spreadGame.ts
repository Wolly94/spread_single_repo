...
export class SpreadGameImplementation implements SpreadGame {
	...
	triggerStart() {
		...
		this.cells = this.cells.map((cell) => {
			...
			const startCellProps = startGameCellUtils.collect(attProps.concat(props));
			return {
				...cell,
	...
	handleEvent(event: SpreadGameEvent) {
		const props = this.perks.flatMap((perk) => {
			return perk.triggers.flatMap((tr) => {
				if (tr.type === "StartGame" && event.type === "StartGame") {
				...
				} else if (tr.type === "TimeStep" && event.type === "TimeStep") {
					return tr.getValue(event, this);
				} else if (tr.type === "Growth" && event.type === "Growth") {
					return tr.getValue(event, this);
				} else if (tr.type === "Move" && event.type === "Move") {
					return tr.getValue(event, this);
				} else if (tr.type === "CapturedCell" && event.type === "CapturedCell")
					return tr.getValue(event, this);
				else if (tr.type === "DefendedCell" && event.type === "DefendedCell")
					return tr.getValue(event, this);
				else if (
					...
					return tr.getValue(event, this);
				else if (tr.type === "CreateBubble" && event.type == "CreateBubble") {
					return tr.getValue(event, this);
				} else return [];
			});
		});
		...
		remProps
			.filter((prop): prop is RaiseEventProps => prop.type === "RaiseEvent")
		...
		if (event.type === "DefendedCell") {
			...
			this.cells[index] = {
				...this.cells[index],
				units:
					this.cells[index].units * conquerProps.unitsInPercentToRemain +
	...
	step(ms: number) {
		...
		this.cells = this.cells.map((cell) => {
			...
			const growthProps = growthUtils.collect(
				this.fromAttachedProps({ type: "Cell", id: cell.id }).concat(props)
			);
			...
			if (ev.data.type === "CollisionEvent" && !ev.data.finished) {
				...
				const currentDefender =
					eventData.before.other.type === "Cell"
						? this.cells.find((c) => c.id === eventData.before.other.val.id)
						: this.bubbles.find((b) => b.id === eventData.before.other.val.id);
	...
	processFight(before: BeforeCollisionState, after: AfterCollisionState) {
		...
		if (
			...
			const newEvent = createCollisionEvent(before, after, this.timePassed);
		...
		const capturedCellEvent: CapturedCellEvent | null = getCapturedCellEvent(
			before,
			after
		);
		...
		this.bubbles.forEach((bubble) => {
			...
			this.cells = this.cells.map((cell) => {
				if (
					...
					const afterFight: AfterCollisionState = {
						bubble: newCurrentBubble !== null ? { ...newCurrentBubble } : null,
	...
	toClientGameState(playerId: number | null = null) {
		...
		const gs: ClientGameState = {
			...
			cells: this.cells.map((cell) => {
				...
				const hideProps =
					playerId !== null ? cellProps.hideProps.get(playerId) : undefined;
