...
const latestMoveTimeStamp = (
	...
	const lastAttackSent = eventHistory
		.filter(
			(ev) => ev.data.type === "SendUnits" && ev.data.sender.id === cell.id
		)
		.slice(-1)[0];
			...
			triggers: [
				{
					...
					getValue: (
						trigger,
						game
					): AttachProps<TimedProps<VisualizeCellProps | CellFightProps>>[] => {
						const res: AttachProps<
							TimedProps<VisualizeCellProps | CellFightProps>
						>[] = game.players.flatMap((p) => {
							const val = getPerkValue(game, name, p.id, values, defaultValue);
							return game.cells
